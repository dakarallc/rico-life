...
<div class="page plan-page">
  ...
  <section class="planItem">
    <div class="inner planItem__inner">
      ...
      <div class="planItem__list">
        <ul>
          ...
          <li><a href="<?php echo home_url(
          	"/plan/?budget=" . $budget[0]->slug,
          ); ?>"><?php echo $budget_name; ?></a></li>
          <?php endif; ?>
          <?php if (!empty($madori_name)): ?>
          <li><a href="<?php echo home_url(
          	"/plan/?layout=" . $madori[0]->slug,
          ); ?>"><?php echo $madori_name; ?></a></li>
          <?php endif; ?>
        </ul>
      </div>
      ...
      // 月々の返済額計算（元利均等返済）
      $monthly_payment =
      	($total_price * ($monthly_interest * pow(1 + $monthly_interest, $total_payments))) /
      	(pow(1 + $monthly_interest, $total_payments) - 1);
      ...
      <div class="simulation-table loan-simulation">
        <table>
          ...
          <tr>
            <th>建物本体価格</th>
            <td><span class="emphasis"><?php echo number_format(
            	$house_price,
            ); ?></span><span class="unit">円</span></td>
          </tr>
          <tr>
