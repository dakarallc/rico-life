...
gulp.task("ejs", (done) => {
  gulp
    .src(["assets/ejs/**/*.ejs", "!" + "assets/ejs/**/_*.ejs"])
    .pipe(ejs({}, {}, {ext:'.html'}))
...
gulp.task('watch', function (done) {
  ...
  gulp.watch('assets/sass/**/*.scss', gulp.task('bs-reload')); //sassが更新されたらbs-reloadを実行
  gulp.watch('assets/ejs/**/*.ejs', gulp.task('ejs')); //ejsが更新されたらgulp ejsを実行
  gulp.watch('assets/js/*.js', gulp.task('bs-reload')); //jsが更新されたらbs-reloadを実行
  // gulp.watch('assets/js/*.js', gulp.task('js-minify')); //jsが更新されたらbs-reloadを実行
  done();
});
